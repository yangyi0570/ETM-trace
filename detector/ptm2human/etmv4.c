...
/**
 * DEF_TRACEPKT(extension, 0xff, 0x00);宏定义展开可以得到：
 * static struct tracepkt tracepktextension
 * {
 *  .name="extension",
 *  .mask=(0xff),   //对于每种类型的包，均有其header byte
 *  .val=(0x00),    // header byte & mask == val 时，说明这个包属于该类型
 *  .decode=decode_extension,  //decode_extension为一个函数指针，指向下面定义的函数
 * }
 * 
 */
...
/**
 * DECL_DECODE_FN(extension)展开为：
 * static int decode_extension(const unsigned char* pkt, struct stream* stream){...}
 */
DECL_DECODE_FN(extension)
{
    int index = 1, cnt;

    switch (pkt[index]) {   //pkt[1]指向了第二个字节，extension包通过第二个字节确定子类型
    case 0: 
        /* async */
        ...
        if (cnt != 11) {
            printf("Payload bytes of async are not correct\n");
            printf("Invalid async packet\n");
            return -1;
        }
        printf("[async]\n");
        //如果是正确的async包，则index=12
        break;
    ...
    case 5:
        ...
        break;  //如果是overflow包，index=2

    default:
        printf("First payload byte of async is not correct\n");
        printf("Invalid async packet\n");
        index = -1;
        break;
    }

    return index;   //如果有错误，则返回index=1
}
...
DECL_DECODE_FN(trace_info)
{
    ...
    for (i = 0; i < 4; i++) {
        data = pkt[index++]; //对header byte后的四个字节进行判断
        plctl |= (data & ~c_bit) << (7 * i);    //多次循环，第一次循环是plctl[6:0]，第二次循环是plctl[12:7]，以此类推
        if (!(data & c_bit)) {
            break;
        }
    }
    if (i >= 1) {
        printf("More than 1 PLCTL field in the trace info packet\n");
        return -1;
    }
    ...
    if (plctl & 1) {
        ...
        if (i >= 1) {
            printf("More than 1 INFO field in the trace info packet\n");
            return -1;
        }
    }
    ...
    if (plctl & 2) {
        ...
        if (i >= 4) {
            ...
            printf("More than 4 KEY fields in the trace info packet\n");
            return -1;
        }
    }
    ...
    if (plctl & 4) {
        ...
        if (i >= 4) {
            ...
            printf("More than 4 SPEC fields in the trace info packet\n");
            return -1;
        }
    }
    ...
    if (plctl & 8) {
        ...
        if (i >= 2) {
            printf("More than 2 CYCT fields in the trace info packet\n");
            return -1;
        }
    }

    printf("[trace info] plctl = 0x%X, info = 0x%X, key = 0x%X, spec = %d, cyct = 0x%X\n",
...
DECL_DECODE_FN(timestamp)
{
    ...
    printf("[timestemp] timestamp = %llu, cycle count = %d, nr_replace = %d\n", ts, count, nr_replace);
...
DECL_DECODE_FN(exception)
{
    ...
    if (pkt[index++] & 1) {
        ...
        printf("[exception] E1:E0 = %d, TYPE = 0x%02X, P = %d\n", EE, TYPE, P);

        if (EE != 1 && EE != 2) {
            printf("Invalid EE in the exception packet\n");
            return -1;
        } else if (EE == 2) {
            ...
            if (ret > 0) {
                ...
                printf("Invalid address packet in the exception packet\n");
                return -1;
            }
        }
...
DECL_DECODE_FN(cc_format_1)
{
    ...
    if (1) {
        ...
        if (i >= 4) {
            printf("More than 4 bytes of the commit section in the cycle count format 1 packet");
            return -1;
        }
    ...
    if (!u_bit) {
        ...
        if (i >= 3) {
            printf("More than 3 bytes of the cycle count section in the cycle count format 1 packet");
            return -1;
        }
    }

    printf("[cycle count format 1] U = %d, COMMIT = %d, COUNT = %d\n", u_bit, commit, count);
...
DECL_DECODE_FN(cc_format_2)
{
    ...
    AAAA = (pkt[1] & 0xf0) >> 4;
    BBBB = pkt[1] & 0x0f;
    printf("[cycle count format 2] F = %d, AAAA = %d, BBBB = %x\n", F, AAAA, BBBB);
...
DECL_DECODE_FN(cc_format_3)
{
    ...
    AA = (pkt[0] & 0x0c) >> 2;
    BB = (pkt[0] & 0x03);
    printf("[cycle count format 3] AA = %d, BB = %x\n", AA, BB);
...
DECL_DECODE_FN(data_sync_marker)
{
    if (pkt[0] & 0x08) {
        ...
        printf("[unnumbered data sync maker] A = %d\n", pkt[0] & 0x07);
    } else {
        /* Numbered data synchronization marker */
        printf("[numbered data sync maker] NUM = %d\n", pkt[0] & 0x07);
    }
...
DECL_DECODE_FN(commit)
{
    ...
    if (i >= 4) {
        printf("More than 4 bytes of the commit section in the commit packet");
        return -1;
    }
    printf("[commit] COMMIT = %d\n", commit);
...
DECL_DECODE_FN(cancel)
{
    ...
    if (!(pkt[index] & 0x10)) {
        ...
        if (i >= 4) {
            printf("More than 4 bytes of the cancel section in the cancel format 1 packet");
            return -1;
        }
        printf("[cancel format 1] M = %d, CANCEL = %d\n", pkt[0] & 0x01, cancel);
        tracer_cancel(&(stream->tracer), pkt[0] & 0x01, cancel);
    } else if (!(pkt[index] & 0x80)) {
        /* cancle format 2 */
        index++;
        printf("[cancel format 2] A = %d\n", pkt[index] & 0x03);
        ...
        case 3:
            ...
            break;
        default:
            printf("Unexpected A in a cancel format 2 packet");
            break;
        }
        ...
        /* cancle format 3 */
        index++;
        printf("[cancel format 3] CC = %d, A = %d\n", (pkt[index] & 0x06) >> 1, pkt[index] & 0x01);
        if (pkt[index] & 0x01) {
            tracer_atom(&(stream->tracer), ATOM_TYPE_E);
        }
...
DECL_DECODE_FN(cond_inst_format_1)
{
    ...
    printf("[conditional instruction format 1] key = %d\n", key);
...
DECL_DECODE_FN(cond_inst_format_2)
{
    ...
    ci = pkt[0] & 0x03;
    printf("[conditional instruction format 2] ci = %d\n", ci);
...
DECL_DECODE_FN(cond_inst_format_3)
{
    ...
    z = pkt[1] & 0x01;
    num = (pkt[1] & 0x7E) >> 1;
    printf("[conditional instruction format 3] z = %d, num = %d\n", z, num);
...
DECL_DECODE_FN(cond_result_format_1)
{
    ...
    for (payload = 0; payload < nr_payloads; payload++) {
        ...
        if (i >= 5) {
            printf("More than 5 payload bytes in the conditional result format 1 packet");
            return -1;
        }
        printf("[conditional result format 1] CI[%d] = %d, RESULT[%d] = 0x%X, KEY[%d] = %d\n",
                payload, CI,
                payload, RESULT,
                payload, KEY);
...
DECL_DECODE_FN(cond_result_format_2)
{
    ...
    K = (pkt[0] >> 2) & 0x1;
    T = (pkt[0] & 0x3);
    printf("[conditional result format 2] K = %d, T = 0x%X\n", K, T);
...
DECL_DECODE_FN(cond_result_format_3)
{
    ...
    printf("[conditional result format 3] TOKEN = 0x%X\n", TOKEN);
...
DECL_DECODE_FN(cond_result_format_4)
{
    ...
    T = (pkt[0] & 0x3);
    printf("[conditional result format 4] T = 0x%X\n", T);
...
{
    unsigned char e_val = pkt[0] & 0x0F;    //8位
    printf("[event] EVENT = 0x%X\n", e_val);
    if (stream->state >= INSYNC) {
        tracer_event(&(stream->tracer), e_val);
    }
...
DECL_DECODE_FN(short_address)
{
    ...
    printf("[short address] address = 0x%016llx, IS = %d\n", address, IS);
...
DECL_DECODE_FN(long_address)
{
    ...
    printf("[long address] address = 0x%016llx, IS = %d\n", address, IS);
...
DECL_DECODE_FN(exact_match_address)
{
    ...
    printf("[Exact match address] QE = %d, address = 0x%016llx, IS = %d\n", QE,
...
DECL_DECODE_FN(context)
{
    ...
    if (data & 1) {
        ...
        printf("[context] P = 1'b1, EL = %d, SF = %d, NS = %d, V = %d, C = %d, VMID = %d, CONTEXTID = 0x%04X\n", EL, SF, NS, V, C, VMID, CONTEXTID);
    } else {
        printf("[context] P = 1'b0\n");
    }
...
DECL_DECODE_FN(address_context)
{
    ...
    printf("[address with context] address = 0x%016llx, IS = %d, EL = %d, SF = %d, NS = %d, V = %d, C = %d, VMID = %d, CONTEXTID = 0x%04X\n", address, IS, EL, SF, NS, V, C, VMID, CONTEXTID);
...
DECL_DECODE_FN(atom_format_5_1)
{
    ...
    default:
        printf("Invalid ABC in an ATOM format 5 packet\n");
        break;
    }
...
DECL_DECODE_FN(atom_format_6_1)
{
    ...
    COUNT = pkt[0] & 0x1f;
    if (COUNT > 20) {
        printf("Invalid COUNT in an ATOM format 6 packet\n");
        return -1;
    }
    printf("[atom format 6] A = %d, COUNT = %d\n", A, COUNT);
...
DECL_DECODE_FN(q)
{
    ...
    if (count_unknown) {
        printf("[Q] type = %d, address = 0x%016llx, IS = %d, count unknown\n", type,
                ADDRESS_REGISTER(&(stream->tracer))[0].address,
                ADDRESS_REGISTER(&(stream->tracer))[0].IS);
    } else {
        printf("[Q] type = %d, address = 0x%016llx, IS = %d, count =%d\n", type,
                ADDRESS_REGISTER(&(stream->tracer))[0].address,
...
int etmv4_synchronization(struct stream *stream)
{
    ...
    for (i = 0; i < stream->buff_len; i++) {
        ...
        if ((c & PKT_NAME(extension).mask) == PKT_NAME(extension).val) {
            ...
            if (p != 12)    //p!=12意味着不是async包，则继续检查下一个包
                continue;
            c = stream->buff[i + 12];   //指向async包的下一个字节开始的byte
            if ((c & PKT_NAME(trace_info).mask) == PKT_NAME(trace_info).val) {
                ...
                if (p > 0) {
                    ...
                    return i; //返回async包在stream中的第i个包的位置
                }
            }
             ...
             */
            printf("No trace info packet right after an a-sync packet\n");
        }
    }
